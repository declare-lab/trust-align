...
class LLM:
    ...
    def generate(
        ...
        if args.azure_openai_api:
            use_azure_chat_completion = (
                "turbo" in self.args.model.lower()
                or "gpt4" in self.args.model.lower()
                or "gpt-4" in self.args.model.lower()
            )
            print(use_azure_chat_completion)

            formatted_prompt = (
                [
                    {
                        "role": "system",
                        "content": "Assistant is a large language model trained by OpenAI that answers questions with proper citations.",
                    },
            ...
            return self._call_azure_openai_api(
                use_azure_chat_completion, formatted_prompt, max_tokens, stop
            )
