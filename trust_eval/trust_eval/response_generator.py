...
class ResponseGenerator:
    ...
    def load_data(
        ...
        if data_file is not None:
            self.eval_data = json.load(open(data_file))[: self.config.quick_test]
        else:
            assert self.data_file is not None
            self.eval_data = json.load(open(self.data_file))[: self.config.quick_test]
    ...
    def generate_responses(
        ...
        if incomplete_doc_list > 0:
            logger.warning(
                f"There are {incomplete_doc_list} questions with incomplete document lists."
            )
    ...
    def _generate_head_prompt(self) -> str:
        ...
        if not self.config.no_demo:
            ...
            if "refusal" in self.config.prompt_file:
                ...
                train_items = [
                    ...
                    *[self.prompt_data["refusal_demos"][idx] for idx in rej_train_ids],
                ]
