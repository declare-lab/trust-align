...
logging.setLevel(colorlog.INFO)
...
DEFAULT_TEMPLATE = "[INST]{examples}\n\n{instruction}[/INST]"
...
class ClusterClassifier:
    def __init__(
        ...
        summary_model_kwargs = None,
        topic_mode="multiple_topics",
        summary_n_examples=30,
        ...
        self.summary_model_kwargs = dict(
        ...
        if summary_model_kwargs:
            self.summary_model_kwargs.update(summary_model_kwargs)
    ...
    def embed(self, texts):
        if len(self.embed_devices) == 1:
            embeddings = self.embed_model.encode(
                ...
                device=self.embed_devices[0]
            )
        else:
            pool = self.embed_model.start_multi_process_pool(target_devices=self.embed_devices)
    ...
    def summarize(self, texts, labels):
        ...
        for label in range(unique_labels):
            ...
            requests.append(self.summary_template.format(
                examples=examples, instruction=self.summary_instruction
            ))
        
        outputs = self.llm.generate(requests, sampling_params)
        ...
        for idx, resp in enumerate(responses):
            ...
            cluster_summaries[idx] = self._postprocess_response(resp)
        logging.critical(f"Number of clusters is {len(cluster_summaries)}")
    ...
    def _postprocess_response(self, response):
        if self.topic_mode == "multiple_topics":
            ...
            try:
                topic = response.split('Demanding value rating')[0].split("Topic:")[1].split("(")[0].strip().strip( r"""!"#$%&'*+,-./:;<=>?@\^_`{|}~""")
            except IndexError:
                logging.warning(f"No topic found for {response}")

            rating_match = re.search(r"Demanding value rating: (\d+)", response)
            if rating_match:
                score = rating_match.group(1).strip()
            else:
                logging.warning(f"No score found for {response}")
            full_output = f"{topic}. Score: {score}"
            return full_output
        else:
            raise ValueError(
    ...
    def save(self, folder):
        folder = Path(folder).joinpath("clusters")
    ...
    def show(self, interactive=False, folder=None):
        df = pd.DataFrame(
            data={
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        if interactive:
            self._show_plotly(df, folder)
        else:
            self._show_mpl(df, folder)

    def _show_mpl(self, df, folder):
        ...
        ax.set_axis_off()
        # fig.show()
        fig.savefig(f"{folder}/mpl_scatter.png", bbox_inches="tight", dpi=300)

    def _show_plotly(self, df, folder):
        fig = px.scatter(
            df,
            x="X",
            y="Y",
        ...
        fig.update_traces(hovertemplate="%{customdata[0]}<br>label=%{marker.color}<extra></extra>")
        ...
        # fig.show()
        fig.write_image(f"{folder}/plotly_scatter.jpeg")
