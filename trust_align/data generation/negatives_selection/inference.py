...
def make_demo(
    ...
    if "{D}" in prompt:
        if ndoc == 0:
            ...
            doc_list = (
                get_shorter_text(item, item["docs"], ndoc, use_shorter)
                if use_shorter is not None
                else item["docs"][:ndoc]
            )
            text = "".join(
...
class LLM:
    ...
    def __init__(self, args):
        ...
        if args.openai_api:
            ...
            self.chat_llm = AzureChatOpenAI(
                deployment_name=model_engine, max_retries=3, model_name=model_name
            )
    ...
    def generate(self, prompt, max_tokens, stop=None):
        ...
        if args.openai_api or args.anthropic_api:
            ...
            if args.openai_api:
                self.prompt_tokens += response.response_metadata["token_usage"][
                    "prompt_tokens"
                ]
                self.completion_tokens += response.response_metadata["token_usage"][
                    "completion_tokens"
                ]

            if args.anthropic_api:
                self.prompt_tokens += response.response_metadata["usage"][
                    "input_tokens"
                ]
                self.completion_tokens += response.response_metadata["usage"][
                    "output_tokens"
                ]
...
def main():
    ...
    # Model and name
    parser.add_argument(
        "--dataset_name", type=str, help="Name of the dataset (for saving)"
    )
    parser.add_argument("--model", type=str, help="Model to use")
    parser.add_argument(
        "--openai_api", type=bool, default=False, help="Whether to use OpenAI API"
    )
    parser.add_argument(
        "--anthropic_api", type=bool, default=False, help="Whether to use Anthropic API"
    )
    parser.add_argument(
        "--vllm", type=bool, default=False, help="Whether to use vllm for acceleration"
    )

    # Decoding
    parser.add_argument(
        "--temperature", type=float, default=0.5, help="Temperature for decoding"
    )
    parser.add_argument(
        "--top_p", type=float, default=1.0, help="Nucleus sampling top-p"
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
        default=300,
        help="Max number of new tokens to generate in one step",
    )
    parser.add_argument(
        "--max_length",
        type=int,
    ...
    parser.add_argument(
        "--num_samples", type=int, default=1, help="Sample multiple answers."
    )
    ...
    if "refusal" in args.prompt_file:
        ...
        pos_train_ids = np.random.choice(
            len(prompt_data["positive_demos"]), ceil(args.shot / 2), replace=False
        )
        rej_train_ids = np.random.choice(
            len(prompt_data["refusal_demos"]), args.shot // 2, replace=False
        )
        ...
        for rej_train_id in rej_train_ids:
            train_items.append(prompt_data["refusal_demos"][rej_train_id])
        ...
        train_ids = np.random.choice(
            len(prompt_data["demos"]), args.shot, replace=False
        )
    ...
    incomplete_doc_list = (
        0  # For some questions there might be fewer than ndoc documents
    )
    ...
    if args.vllm:
        prompts = [
            item["prompt"] for item in infer_data for _ in range(args.num_samples)
        ]
        if args.openai_api:
        ...
        for i in range(len(infer_data)):
            ...
            infer_data[i]["output"] = (
                output_array if len(output_array) > 1 else output_array[0]
            )
    ...
    if args.openai_api:
        ...
        infer_data["total_cost"] = (
            llm.prompt_tokens / 1000 * p_price + llm.completion_tokens / 1000 * c_price
        )

        logger.info(
            f"Unit price (Oct 16, 2023, prompt/completion) per KToken: {p_price}/{c_price}"
        )
    ...
    if args.anthropic_api:
        ...
        infer_data["total_cost"] = (
            llm.prompt_tokens / 1000000 * p_price
            + llm.completion_tokens / 1000000 * c_price
        )

        logger.info(
            f"Unit price (July 16, 2024, prompt/completion) per MToken: {p_price}/{c_price}"
        )
        logger.info(f"Total cost: %.1f" % (infer_data["total_cost"]))
