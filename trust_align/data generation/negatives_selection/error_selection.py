...
def is_refusal_prec_error(answer, pattern):
    refusal = (
        fuzz.partial_ratio(normalize_answer(REFUSAL_FLAG), normalize_answer(answer))
        > REFUSAL_FUZZ_THRESHOLD
    )
...
def not_refuse(answer):
    return (
        not fuzz.partial_ratio(normalize_answer(REFUSAL_FLAG), normalize_answer(answer))
        > REFUSAL_FUZZ_THRESHOLD
    )
...
def compute_qampari_f1(sample, pattern):
    ...
    preds = [
        x.strip() for x in sample["output"].rstrip().rstrip(".").rstrip(",").split(",")
    ]
    ...
    logger.debug(
        f"{normalized_preds=}, {answers=}, {pattern=}, {matched=}, {rec_top5=}"
    )
    ...
    filtered_flat_answers = [
        sample for sublist in filtered_answers for sample in sublist
    ]
    p_matched = []
    for p in normalized_preds:
        if p in filtered_flat_answers:
            ...
            p_matched.append(0)
    prec = (
        sum(p_matched) / len(normalized_preds) if len(normalized_preds) > 0 else 0
    )  # how many of output is correct
    ...
    return (
        ...
        upvoted_content_labels,
        downvoted_content_labels,
    )
...
def compute_autoais(sample, qampari=False, at_most_citations=None):
    """
    ...
    if qampari:
        sents = [
            sample["question"] + " " + x.strip()
            for x in sample["output"].rstrip().rstrip(".").rstrip(",").split(",")
        ]
        # print(f'{sents=}')
    else:
        sents = sentence_splitter.tokenize(sample["output"])
    ...
    for sent_id, sent in enumerate(sents):
        ...
        if len(ref) == 0:
            ...
            joint_passage = "\n".join(
                [_format_document(sample["docs"][psgs_id]) for psgs_id in ref]
            )
        ...
        if joint_entail and len(ref) > 1:
            ...
            for psgs_id in ref:
                ...
                if not nli_result:
                    ...
                    passage = "\n".join(
                        [
                            _format_document(sample["docs"][pid])
                    ...
                    subset_coverage = np.bitwise_or.reduce(
                        [sample["docs"][pid]["answers_found"] for pid in subset_exclude]
                    )
                    contained = False
                    for i in range(len(subset_coverage)):
                        if (
                            subset_coverage[i] == 1
                            and sample["docs"][psgs_id]["answers_found"][i] == 1
                        ):
                            contained = True
                            break
    ...
    if sent_mcite > 0 and sent_mcite_support > 0:
        ...
        logger.info(
            ...
            % (100 * sent_cited, 100 * sent_supported, 100 * overcite)
        )
    else:
        sent_cited = 0
        sent_supported = 0
...
def process_samples(data, dataset_name=None):
    ...
    for sample_idx, sample in tqdm(
        enumerate(data), desc=f"Processing {dataset_name} questions", total=len(data)
    ):
        question = sample["question"]
        docs = sample["docs"]
        ...
        if not_invalid_qns(pattern) and not_refuse(answer):
            # calculate eval metrics
            # coverage, upvoted_content_labels, downvoted_content_labels
            if dataset_name == "asqa":
                coverage, upvoted_content_labels, downvoted_content_labels = (
                    compute_str_em(sample, pattern)
                )
            elif dataset_name == "eli5":
                coverage, upvoted_content_labels, downvoted_content_labels = (
                    compute_claims(sample, pattern)
                )
            elif dataset_name == "qampari":
                coverage_stats, upvoted_content_labels, downvoted_content_labels = (
                    compute_qampari_f1(sample, pattern)
                )
            ...
            citation, upvoted_cite_labels, downvoted_cite_labels = compute_autoais(
                sample, qampari=(dataset_name == "qampari")
            )
            ...
            if is_refusal_prec_error(answer, pattern):
                ...
                upvoted_content_labels, downvoted_content_labels = [], [
                    [i, sent] for i, sent in enumerate(sents)
                ]
                upvoted_cite_labels, downvoted_cite_labels = [], []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_name", type=str, help="Name of the dataset (asqa, qamapri, eli5)"
    )
    ...
    samples_to_errors, errors_to_samples, labelled_data = process_samples(
    ...
    save_data_to_json(
        [samples_to_errors, errors_to_samples], args.output_dir, file_name
    )
